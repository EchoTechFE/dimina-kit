...
`@dimina-kit/workbench/main` 的连接层为主进程提供一组承重原语：`Connection` / `ConnectionRegistry`、随连接寿命确定性拆除的资源归属、以及一个观测用的 `debugTap`。它是 devtools 主进程（以及下游 host）做"按 webContents 归属资源、按 webContents 死亡级联清理"的底座。

本文面向要理解或使用这一层的工程师，回答：它是什么、解决什么、怎么用、有哪些必须知道的语义与红线。

实现位置：
- `packages/workbench/src/main/connection.ts` — `Connection` / `ConnectionRegistry`
- `packages/workbench/src/main/disposable.ts` — `DisposableRegistry`
- `packages/workbench/src/main/debug-tap.ts` — `debugTap`
...
`createConnectionRegistry()` 产出一个 registry。devtools 把它作为 `WorkbenchContext.connections` 字段，由真实接线点（app 启动、view-manager、bridge-router、各 CDP service）按需 `acquire`。
...
- **caller 供 `ts`**：每条 entry 的时间戳由调用方打。workbench 包内**禁用 `Date.now()`**（determinism / resumability），所以时间由 caller 传入。
