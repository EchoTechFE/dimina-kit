 ...
 * - WorkbenchApp is exercised directly for cases that need to bypass the
 *   electron lazy import; workbench() public entry covers issue #1.
 */

import { describe, expect, it, vi } from 'vitest'
import { workbench } from '../workbench.js'
import type { WorkbenchOptions } from '../types.js'
import { WorkbenchApp } from '../internal/workbench-app.js'
...
import { WorkbenchChannel } from '../shared/protocol.js'
import type {
...
// ─────────────────────────────────────────────────────────────────────────
// X1 — R1/C1: public workbench(config, options?) entry behaviour
// ─────────────────────────────────────────────────────────────────────────

describe('X1 — workbench() entry & WorkbenchOptions injection', () => {
	it('resolves when full electron + ipcMain are injected', async () => {
		const electron = makeElectron()
		const ipcMain = makeIpcMain()
		await expect(workbench({}, { electron, ipcMain })).resolves.toBeUndefined()
	})
	...
	it('rejects with an error mentioning electron when running without injection (vitest env)', async () => {
		...
		await expect(workbench({})).rejects.toThrow(/electron/i)
	})

	it('rejects when only electron is injected but ipcMain is missing', async () => {
		const electron = makeElectron()
		await expect(
			workbench({}, { electron } as WorkbenchOptions),
		).rejects.toThrow(/electron|ipcMain/i)
	})

	it('exports WorkbenchOptions type from the package surface', async () => {
...
describe('X7 — trust ref-count preserves baseline auto-trust', () => {
	it('runtime.windows.trust(declaredWindow).dispose() leaves the window still trusted', async () => {
		...
		const rt = app.runtime
		const declared = electron.__windows[1]!
		// Resolve the invoke handler that ipcMain.handle('__workbench:invoke', ...)
		// was registered with, so we can probe trust through the public surface.
		const invokeHandler = ipcMain.__handlers.get(WorkbenchChannel.Invoke)!
		...
		expect(beforeResp.ok === false && beforeResp.error?.code === 'WORKBENCH_UNTRUSTED_SENDER').toBe(false)
		...
		expect(afterResp.ok === false && afterResp.error?.code === 'WORKBENCH_UNTRUSTED_SENDER').toBe(false)
	})
})
...
// ─────────────────────────────────────────────────────────────────────────
// X9 — R8: empty remoteName from WorkbenchRemoteError preserved (?? not ||).
// ─────────────────────────────────────────────────────────────────────────

describe('X9 — empty remoteName preserved via ??', () => {
	it('host throwing new WorkbenchRemoteError("", msg, code) preserves remoteName === ""', async () => {
		...
		const transport = new WireTransport({
			...
			declaredEvents: () => [],
			invokeHost: async () => {
				throw new WorkbenchRemoteError('', 'something exploded', 'E_FOO')
			},
			invokeSimulator: async () => null as never,
		})
		transport.start()
		const handler = ipcMain.__handlers.get(WorkbenchChannel.Invoke)!
