...
import type { HostEvent, JsonValue, WorkbenchConfig, WorkbenchOptions } from './types.js'
import { assertEventDeclared, validateConfig, workbench } from './workbench.js'

/**
 * Phase 1 contract tests for the workbench package's top-level surface.
 *
 * - validateConfig: pure static validation; no Electron, no IPC.
 * - assertEventDeclared: tiny guard used by publish()-time checks.
 * - workbench(config): top-level entry; Phase 1 only validates config and
...
describe('workbench(config)', () => {
	...
	// removed when Phase 2 wired the actual WorkbenchApp.)
	//
	...
	it('Phase 2 contract: resolves for a valid empty config', async () => {
		...
		await expect(workbench({}, options)).resolves.toBeUndefined()
	})

	it('Phase 2 contract: invokes config.setup(runtime) once', async () => {
		const setup = vi.fn()
		await workbench({ setup }, makeInjectedOptions())
		expect(setup).toHaveBeenCalledTimes(1)
	})

	it('Phase 2 contract: setup throwing causes workbench() to reject with the same error', async () => {
...
// ── workbench(config, options) — DI / lazy-import contract ──────────────

/**
 * Minimal fake Electron + ipcMain used to exercise the explicit-injection
 * branch of `workbench(config, options)`. Mirrors the fakes used by
 * `workbench-app.test.ts` but kept local here to avoid coupling test
...
function makeInjectedOptions(): WorkbenchOptions {
	return {
...
describe('workbench(config, options) — DI', () => {
	it('with fully-injected electron + ipcMain: resolves and exercises the fake path', async () => {
		...
		await expect(workbench(app, { electron, ipcMain })).resolves.toBeUndefined()
	...
	it('with electron injected but ipcMain missing: still rejects on lazy ipcMain lookup', async () => {
		...
		await expect(
			workbench({}, { electron: createFakeElectron() }),
