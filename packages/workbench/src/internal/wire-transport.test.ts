...
// 但同时上层 workbench-app 也可能想统一 error mapping。当前 spec 让
// WireTransport 兼做 error → wire-failure 序列化，意味着 `WorkbenchRemoteError`
// 类的 code-preservation 语义被分散到两处实现（workbench-app 与 wire-transport），
...
function makeHarness(opts: {
	...
	return {
		...
		invokeSimulator,
		getInvokeHandler: () => {
			const h = ipcMain.handlers.get(WorkbenchChannel.Invoke)
			if (!h) throw new Error('invoke handler not registered')
			return h
		},
		getProbeHandler: () => {
			const h = ipcMain.handlers.get(WorkbenchChannel.Probe)
...
describe('WireTransport — start()', () => {
	it('registers ipcMain.handle for __workbench:invoke and __workbench:probe each exactly once', () => {
		const h = makeHarness()
		h.transport.start()
...
describe('WireTransport — invoke handler: kind routing', () => {
	it('kind: host → calls invokeHost(name, args) and wraps in InvokeSuccess; does NOT call invokeSimulator', async () => {
		...
		expect(h.invokeHost).toHaveBeenCalledTimes(1)
		expect(h.invokeHost).toHaveBeenCalledWith('svc', [10, 20])
		expect(h.invokeSimulator).not.toHaveBeenCalled()
	})
	...
	it('kind: simulator → calls invokeSimulator(name, args); does NOT call invokeHost', async () => {
		...
		expect(h.invokeSimulator).toHaveBeenCalledWith('getStorage', ['key'])
		expect(h.invokeHost).not.toHaveBeenCalled()
	})
	...
	it('unknown kind → InvokeFailure with code UNKNOWN_KIND; does NOT dispatch', async () => {
		...
		expect(res.ok).toBe(false)
		expect(res.error.code).toBe('WORKBENCH_UNKNOWN_KIND')
...
describe('WireTransport — invoke handler: error serialization', () => {
	...
	// codex review #3: host 抛 WorkbenchRemoteError 重抛代理调用结果时，原 remoteName
	// 必须被保留 —— 不能被中间环节的 invoke name 覆盖。
	it('host throwing WorkbenchRemoteError preserves original remoteName + code (codex #3)', async () => {
		const h = makeHarness({
			trustedIds: [1],
			invokeHost: async () => {
				throw new WorkbenchRemoteError('downstream.svc', 'upstream failed', 'E_UPSTREAM')
			},
		})
		h.transport.start()
		const invoke = h.getInvokeHandler()
	...
	// codex review #9 (R8): WorkbenchRemoteError 携带显式空字符串 remoteName 时
	// 表达 "未知来源" 的意图，不能被 invokeName 友好覆盖（`||` → `??`）。
	it('host throwing WorkbenchRemoteError with empty remoteName preserves "" (codex #9 — ?? over ||)', async () => {
		const h = makeHarness({
			trustedIds: [1],
			invokeHost: async () => {
				throw new WorkbenchRemoteError('', 'msg', 'E_FOO')
			},
		})
		h.transport.start()
		const invoke = h.getInvokeHandler()
...
describe('WireTransport — invoke handler: request shape validation', () => {
	...
	it('request not a plain object → BAD_REQUEST and does NOT dispatch', async () => {
		...
		expect(res.ok).toBe(false)
		expect(res.error.code).toBe('WORKBENCH_BAD_REQUEST')
...
describe('WireTransport — event push', () => {
	it('publish on bus fans out to every non-destroyed trusted webContents via send(__workbench:event, envelope)', () => {
		...
		expect(wc1.send).toHaveBeenCalledWith(WorkbenchChannel.Event, {
			name: 'e1',
			payload: { x: 1 },
		})
		expect(wc2.send).toHaveBeenCalledTimes(1)
		expect(wc2.send).toHaveBeenCalledWith(WorkbenchChannel.Event, {
			name: 'e1',
			payload: { x: 1 },
		})
	})
