/**
 * Phase 3a contract tests for `createWorkbenchClient()` + `readBridgeFromGlobal()`.
 *
 * Source-of-truth referenced (and only these):
 *   - `src/shared/protocol.ts` (WorkbenchBridge, envelope shapes, version)
 *   - `src/errors.ts` (WorkbenchClientNotReadyError, WorkbenchRemoteError)
 *   - `src/client/index.ts` JSDoc on WorkbenchClient + createWorkbenchClient
 *
 * The client only talks to whatever the bridge global exposes. So instead of
 * mocking `electron`, we install a fake `WorkbenchBridge` directly onto
...
interface FakeBridge extends WorkbenchBridge {
...
function makeFakeBridge(opts: FakeBridgeOptions = {}): FakeBridge {
	...
	const bridge: FakeBridge = {
		// `version` typing on WorkbenchBridge narrows to the literal constant,
...
function installBridge(bridge: WorkbenchBridge, name = DEFAULT_BRIDGE_GLOBAL): void {
...
// ── createWorkbenchClient: ready() ───────────────────────────────────────

describe('createWorkbenchClient.ready()', () => {
	it('rejects with WorkbenchClientNotReadyError when bridge is missing', async () => {
	...
	it('uses the configured globalName option', async () => {
		...
		const client = createWorkbenchClient({ globalName: '__custom_bridge' })
		await expect(client.ready()).resolves.toBeUndefined()
	})

	it('rejects with WorkbenchClientNotReadyError on major version mismatch', async () => {
...
describe('createWorkbenchClient.invoke()', () => {
	it('forwards to bridge.invoke with kind="host", name, and args', async () => {
		...
		const client = createWorkbenchClient<DemoHostServices, NoEvents>()
		await client.ready()
	...
	it('rejects with WorkbenchRemoteError on ok=false', async () => {
		installBridge(
			makeFakeBridge({
				invokeImpl: async () => ({
					ok: false,
		...
		const client = createWorkbenchClient<DemoHostServices, NoEvents>()
		await client.ready()
		await expect(client.invoke('boom')).rejects.toBeInstanceOf(WorkbenchRemoteError)
	})

	it('WorkbenchRemoteError carries remoteName + message + code', async () => {
		installBridge(
			makeFakeBridge({
				invokeImpl: async () => ({
					ok: false,
		...
		try {
			...
			const e = err as WorkbenchRemoteError
	...
	it('omits code when failure envelope did not carry one', async () => {
		...
		try {
			...
			expect((err as WorkbenchRemoteError).code).toBeUndefined()
		}
	})

	it('throws / rejects WorkbenchClientNotReadyError when bridge is absent', async () => {
		// CONTRACT-AMBIGUOUS: WorkbenchClient JSDoc does not state whether
		// invoke() pre-ready must throw vs queue. We pick the strict
		// interpretation — no bridge means no possible IPC channel, so
		// invoke() surfaces a `WorkbenchClientNotReadyError` (matching the
...
// ── createWorkbenchClient: on() ──────────────────────────────────────────

describe('createWorkbenchClient.on()', () => {
	it('subscribes via bridge.onEvent and filters by event.name', async () => {
		...
		const client = createWorkbenchClient<DemoHostServices, typeof events>()
		await client.ready()
	...
	it('rejects / throws WorkbenchClientNotReadyError when bridge is absent', () => {
		...
		// throws synchronously with a WorkbenchClientNotReadyError.
		const ev = defineEvent<JsonValue>('demo.no-bridge')
		// eslint-disable-next-line @typescript-eslint/no-unused-vars
		const events = [ev] as const
		const client = createWorkbenchClient<DemoHostServices, typeof events>()
		expect(() => client.on(ev, () => {})).toThrow(WorkbenchClientNotReadyError)
	})
})
