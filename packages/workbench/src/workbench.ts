...
import type { WorkbenchAppOptions } from './internal/workbench-app.js'
import type { WorkbenchConfig, WorkbenchOptions } from './types.js'

/**
 * `workbench(config, options?)` 是 framework 唯一入口（见 README §3）。
 *
 ...
 *   `workbench()` 同样 resolve（host 的 main 文件不需 await 阻塞）。
 *
 * `options` 是测试 / 非 Electron 环境用的注入点（见 {@link WorkbenchOptions}）。
 * 生产路径不传 options：framework `await import('electron')` 取真 `ipcMain` /
 * `BrowserWindow` / `WebContentsView`。
 */
export async function workbench(config: WorkbenchConfig, options?: WorkbenchOptions): Promise<void> {
	...
	const app = new WorkbenchApp(config, resolved)
	await app.start()
}

/**
 * 把公共 `WorkbenchOptions` 解析成 internal {@link WorkbenchAppOptions} —— 在
 ...
 */
async function resolveAppOptions(opts?: WorkbenchOptions): Promise<WorkbenchAppOptions> {
	...
	try {
		...
		throw new Error(
			'workbench(): unable to load electron — pass options.electron / options.ipcMain '
	...
	if (!resolvedIpcMain || !resolvedElectron.BrowserWindow || !resolvedElectron.WebContentsView) {
		throw new Error(
			'workbench(): loaded "electron" but it does not expose the main-process surface '
...
function buildAppOptions(
	...
	opts: WorkbenchOptions | undefined,
): WorkbenchAppOptions {
	const wireTransport: WorkbenchAppOptions['wireTransport'] = { ipcMain }
	const out: WorkbenchAppOptions = { electron, wireTransport }
	if (opts?.trustedWebContents) {
		(out.wireTransport as Mutable<NonNullable<WorkbenchAppOptions['wireTransport']>>)
 ...
 */
export function validateConfig(config: WorkbenchConfig): void {
	if (config === null || typeof config !== 'object') {
		throw new TypeError('workbench(config): config must be an object')
	}
