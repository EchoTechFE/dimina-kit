...
> 原子下发**、**B3 tab 保活归属**。
>
> 实现位置约束：A5 全部落在 `packages/electron-deck`（wire / ControlBus / scope）；
> 不碰 `dimina/` 子模块。**安全攸关 / 需真机或 e2e 实证** 的点全文用 🔒 / 🧪 标注。

---

## 0. 现状基线（只读结论）

读过的现有实现，作为改造的起点与「确切破坏面」依据：
...
- `src/internal/deck-app.ts`
  - **每个 wc 当前没有 per-wc Scope**。trust 走 `trustSet`（refcount），寿命走
    `trackedWindows` + `win.on('closed', ...)`（:611）+ registry。Scope 主要用在
    `connection.ts` / layout demo 的 `main.scope` / `session = main.scope.child()`，
    **尚未与 wc 一一绑定**。🔒 这是 A5-1 第 3 点要补的缺口。
...
> ⚠️ 已按 codex 第3轮统一。grant 闸**只存在于 ControlBus.dispatch**，所以任何**布局 / 特权
...
> 与 `architecture.md` §2.4 的同名硬约束一致。
...
2. **deck-app 默认 invoke 路由 vs ControlBus 路由**（⚠️ 已按 codex 第3轮裁为硬约束，非「待确认」）：
   ...
   `architecture.md` §2.4）。两条路由的边界**在接线处（deck-app.ts:554）写死**：特权名只走
