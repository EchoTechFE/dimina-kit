...
- **Node 端也有常驻 pool(`./pool-node`),devtools(devkit)已整体切换到它**(2026-07-02 七):产物与 dmcc 逐字节等价(含 sourcemap、自定义 fileTypes),watch 热重编 1.6–4×(dmcc 每次 build 重建并销毁 3 个 worker_threads,我们常驻)。经 5 个内置 demo + CDP 真机渲染交互实证。见下「Node 常驻 pool 与 devtools 接入」。
...
## Node 常驻 pool 与 devtools(devkit)接入

> 目标:Electron devtools 不再直接依赖 dmcc npm 包,统一消费 `@dimina-kit/compiler`;同时把"常驻 worker"的收益带到 Node 端的 watch 重编。

### 为什么 Node 端还能比 dmcc 快

dmcc 的 `build()` **每次都新建 3 个 worker_threads、编完即销毁**(`index.js` 的 `runCompileInWorker`:`new Worker(...)` → `worker.terminate()`;它的 `workerPool` 只是并发限流器,不复用 worker)。每个新 worker = 新 V8 isolate,要重新 import sass/postcss/cssnano/autoprefixer/@vue/compiler-sfc/oxc 原生 binding/esbuild(含重启 esbuild service 子进程)+ JIT 重热。watch 场景每次保存都重付这笔税。

`./pool-node` 复刻 dmcc 磁盘流程(主线程 `setupCompile` 写共享 staging → 3 stage worker 并发写不相交文件 → `publishToDist` 拷到 `outputDir/{appId}`),但 worker **常驻**:上述初始化只付一次,每次重编只做 `resetCompilerState` + 真正的编译。stage 内编译代码与 dmcc 同源,省的全是基建。

### 实测(5 个内置 demo,`dimina/fe/example/*`)

同一已热进程内各连跑 4 次(冷 + 3 热),sourcemap 关(dmcc 默认):

| demo | 页面数 | dmcc 冷 | dmcc 热 | pool 冷 | pool 热 | 热编提速 |
| --- | --- | --- | --- | --- | --- | --- |
| base | 39 | 1585ms | 826ms | 844ms | **207ms** | 4.0× |
| weui | 54 | 2329ms | 2716ms | 3521ms | **763ms** | 3.6× |
| subpackages | 2+分包 | 1159ms | 1497ms | 1473ms | **548ms** | 2.7× |
| vant | 13 | 2258ms | 1918ms | 3405ms | **1070ms** | 1.8× |
| taro-todo | 1 | 886ms | 849ms | 1122ms | **546ms** | 1.6× |

读法:① dmcc 热≈冷甚至更慢(weui/subpackages)——worker 层零复用,没有任何东西随次数摊薄;② **pool 冷启更慢**(3 worker 首次加载 bundle 的一次性代价)——devtools"开一次改无数次"的场景下划算;③ 真实 devkit watch 重编延迟(touch 源文件→onRebuild):base ~240ms、weui ~245ms、vant ~466ms。

### 正确性

- **与 dmcc 逐字节等价**:`npm run test:pool-node` 先跑 dmcc build×2 学习确定性(scoped id/esbuild 变量命名固有随机),pool 冷+热两次产物在 dmcc 确定的文件上逐字节相同、资源按内容哈希匹配、`{appId,name,path}` 返回值逐键相同;sourcemap(logic.js.map 的 sources/sourcesContent)与 dmcc 一致。
- **能力面**:dmcc `build()` options 全集 = `sourcemap` + `fileTypes`,两者都支持并实测(自定义 `.qdml/.qdss/<qds>` fixture 与 dmcc 产物等价,devkit 全链路可用)。
- **CDP 真机验证**:Playwright + headless Chromium 加载 devkit H5 容器,base/taro-todo/weui 三 demo 应用列表→启动→页面渲染→点击导航→输入交互(todo 添加/勾选)→**改源码 wxml → watch 重编 → live-reload 自动出新文案**,全程 0 页面错误。

### 落地要点(踩坑记录)

- **sourcemap 接缝**:dmcc 的 logic-compiler 只通过 worker 的 `parentPort` message 接收 sourcemap 开关,而本包 shim 了 worker_threads(`isMainThread=true`)把这条路短路了 → build 期给 logic-compiler 追加 `__setEnableSourcemap` 导出,`runStage('logic')` 前显式设置。
- **fs 用原生、worker_threads 仍 shim**:node 构建不 alias `node:fs`(二进制资源不坏),但 worker_threads 保持 shim 以关掉 dmcc 的自绑定 handler;pool/worker 用 `createRequire('node:worker_threads')` 拿真实 Worker/parentPort。
- **dmcc 返回值语义坑**:dmcc 返回 `mainPages[1].path` 是因为它的 style task 先把 `app` unshift 进了共享数组(`[1]` = 原第一页);本包 style stage 不变异数组,等价写法是 `mainPages[0]`。
- **健壮性**(经对抗审查修复):死 worker 懒复活(否则 postMessage 到已退出的 worker 不抛不回,后续 build 永挂);build 串行链提到模块级(`setupCompile`/`publishToDist` 走 dmcc 进程级全局单例,多 pool 实例也必须互斥);dispose 后 build 明确 reject。
- **devkit fork 的 `.ts` 入口**:Node 22.18 才默认 type-stripping,22.6–22.17 需显式 `--experimental-strip-types`,更低版本直接给出可操作的报错(用编译后的 dist 入口)。
- **devkit 多 session bug**(CDP 测试抓到):`fe/server.js` 的 express `app` 是模块级单例,同进程第二次 `openProject` 的 `/appList.json` 命中第一个 session 的 handler → "暂无应用"。修:每次 `start()` 建独立 express 实例,live-reload/proxy 改显式注册。
...
- **2026-07-02(七)**:**Node 常驻 pool(`./pool-node`)+ devtools/devkit 整体切换**。pool 复刻 dmcc 磁盘流程但 worker 常驻(dmcc 每次 build 重建并销毁 3 worker_threads,`runCompileInWorker` 实证);产物与 dmcc 逐字节等价(含 sourcemap、fileTypes,`test:pool-node` 双跑学习法)。devkit `loadCompiler` 切到本包,删除 `@dimina/compiler` 依赖(本包声明全部 14 个运行时 externals 自足)。5 demo 实测热重编 1.6–4×、watch 重编 ~240–570ms;CDP(Playwright)实证渲染/点击/输入/live-reload 全链路。对抗审查修 4 项(entryPage 语义/死 worker 复活/跨实例串行/strip-types 版本门);CDP 抓到并修 devkit express 单例多 session bug。
