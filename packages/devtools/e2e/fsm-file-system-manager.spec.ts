...
import { openProjectInUI, pollUntil } from './helpers'
 ...
 * loads `pageFrame.html`), not in the simulator shell document — query every
 * live page frame and return the first non-empty `.fsm-result` text.
 */
...
test.describe('wx.getFileSystemManager() async parity + saveImageToPhotosAlbum path validation (e2e)', () => {
  test('real service<->container round trip: async FSM works, sync FSM throws, invalid save-image path fails without downloading', async ({ electronApp, mainWindow }) => {
