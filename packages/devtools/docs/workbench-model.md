...
`HS` 上界是 `Record<string, (...args: JsonValue[]) => unknown>`：跨进程 RPC 的 args 必须 JSON-safe，所以 host 在 `hostServices` 写的 narrower handler 签名（如 `(p: { code: string }) => ...`）需要保证参数结构对齐 JSON。narrower 类型由 `Parameters<HS[K]>` 推断。
...
- `beforeClose` 超时 → log error 后继续 shutdown 流程（不阻止关闭）；如需阻断关闭，host 在 `beforeClose` 内自行抛错并自管 UX
