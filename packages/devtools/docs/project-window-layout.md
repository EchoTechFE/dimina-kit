...
> **历史**：项目窗口布局早期是写死的「三栏 FrameTree」——`compileProjectWindowLayout`
> （`LayoutState` → frame 树两段纯函数管线）+ `frame-tree.tsx` 递归渲染器 + `devtoolsPosition` ×
> `simulatorAlignment` 的 6 种 mode + 一排 preset 工具栏（可见性/对齐/devtools 位置 toggle）。
> 那套（`layout/compile.ts` / `layout/frame-tree.tsx` / `layout/types.ts` 的 `CellId`/`Frame*`/
> `OuterSize`/`ProjectWindowLayout` 词汇 / `components/layout-controls.tsx` / `BottomDebugPanel`
> 自带 TABS 栏 / `dockableMode` flag）**已整体删除**，由本文描述的 dockable 布局取代。位置/对齐
> preset 由自由拖拽 re-dock 取代；6 种 mode 概念不再存在。
...
- **七个 dock 面板**：`simulator` / `editor` / `wxml` / `appdata` / `storage` / `console` /
  `compile`。其中 `wxml`/`appdata`/`storage`/`compile` 是原 `BottomDebugPanel` 单一 debug 区拆出
  的**独立 dock 面板**，各自经 `DebugTabContent` 渲染。
- **面板分两类**：
...
`state.dockTree` + `setDockTree(serialized)`。旧的 `simulatorVisible` / `editorVisible` /
`debugVisible` / `simulatorAlignment` / `devtoolsPosition` / `dockableMode` 五字段那套**已不存在**
——面板的显隐/排布全在 dock 树里，由用户拖拽决定。
...
| `src/renderer/modules/main/features/bottom-debug-panel/bottom-debug-panel.tsx` | `DebugTabContent`（单 tab 内容渲染器，被四个 DOM debug 面板 wxml/appdata/storage/compile 复用；console 是原生 WCV 不走它）+ `BottomDebugPanelProps`（旧自带 TABS 栏的 `BottomDebugPanel` 组件本体已删）|
