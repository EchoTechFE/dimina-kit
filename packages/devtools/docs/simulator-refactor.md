...
> 本文是 simulator 运行时的协议与契约参考。devtools 只有这一套运行时（native-host）：Electron 把自己当成 iOS/Android 的同类 native 宿主，复用 `@dimina/service` + `@dimina/render` 两个 bundle，让它们在 Electron 里走 dimina-fe 为真机预留的 Native 分支（`globalThis.DiminaServiceBridge.*` / `window.DiminaRenderBridge.*`）。
>
...
- 环境检测：`dimina/fe/packages/common/src/core/utils.js` `isWebWorker` —— native-host 的 `@dimina/service` bundle 也走这条 Worker-form 检测（loader 的 `importScripts` 路径，见 §5），由 `service-host/preload.cjs` 的 `importScripts` shim 满足
...
> native-host 不像真机那样有"原生同步"语义，也不依赖 `SharedArrayBuffer` / `Atomics.wait` 这类 Worker 阻塞机制——它在 service-host 窗口内用 `sync-api-patch.ts`（`:20-40`）把 `*Sync` patch 成 `sync-impls/` 本地实现。
...
bridge-router 的 `handleContainerMsg` 在 `consoleLog` 分支调 `ctx.guestConsole?.emit(msg.body)`（`bridge-router.ts:822-829`）。`guestConsole` 由 automation 服务设置（`automation/index.ts:88-98`），把它重广播为 `App.logAdded` 事件。render-host preload 走同一条 `consoleLog` 通道（`source: 'render'`）。
...
| 文件 | 角色 |
|---|---|
| `src/service-host/preload.cjs` | service 窗口 preload：注入 `globalThis.DiminaServiceBridge`、`importScripts` shim、guest console 捕获 |
