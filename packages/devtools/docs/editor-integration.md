...
editor 是 dockable 布局里的 `editor` DOM 面板。**与旧 OpenSumi 不同**：editor 是普通 React 子节点，由 React 自行挂载/卸载，**不是 overlay、不发 bounds、不经 view-manager**。`<DockView>` 经 `renderDomPanel('editor')` 直接渲染 `<MonacoEditor/>`，不挂任何 native anchor。整套布局里仍是主进程 overlay 的只有两个原生面板——simulator（设备 WCV）与 console（Console DevTools，`view:simulator:devtools-bounds` + `setSimulatorDevtoolsBounds`），各由一个 `view-anchor`（`@dimina-kit/view-anchor`）锚点同步 bounds——与 editor 无关。布局拓扑/拖拽 re-dock/序列化逻辑见 `project-window-layout.md`。
