...
> 编辑器是 **renderer 内嵌的 Monaco 组件**，占据项目窗口三宫格的 editor 区。
> 配套文档：[`project-window-layout.md`](./project-window-layout.md) —— editor 是该布局抽象层管理的三个 cell 之一（simulator / **editor** / debug）。

## 摘要（TL;DR）

dimina-kit devtools 的代码编辑器是 **Monaco**（VS Code 的编辑器组件），作为一个普通 React 组件直接挂在主 renderer 里，占据三宫格的 editor 区。它读写当前活跃 dimina 项目的文件（经沙盒化的 `project:fs:*` IPC），提供 wxml/wxss 语法高亮 + wxml 补全/悬浮 + 主题同步 + 自动保存（debounce）与保存状态指示。

> **历史背景**：编辑器早期方案是把 OpenSumi-lite 作为独立 `WebContentsView` overlay 经 `dmieditor://` 自定义 protocol 嵌入，后整体迁移到 Monaco。OpenSumi 期遗留（protocol / fs-bridge / theme-bridge / editor preload / `view-manager.attachEditor` / `_spike/opensumi-lite` / `editor-bundle`）均已移除。换 Monaco 的理由：dimina 此前已关掉 OpenSumi 的扩展宿主（`noExtHost:true`），不使用 VS Code 扩展生态，却付着完整 IDE 框架的成本（数十 MB bundle、自定义 protocol、Web Worker 扩展宿主、webpack4、fork+submodule 维护）。dimina-devtools 定位是**可被 host 嵌入的、以 simulator+调试为中心的轻量 SDK**，编辑器只需"好用的代码编辑 + wxml 智能"，Monaco 正是这个尺寸的原语（也是后续自建 AI 编辑平台的基座）。
...
┌──────────────────── 主 renderer（React）────────────────────────────┐
│  project-runtime.tsx → FrameTree → editor cell 渲染：                │
│    <MonacoEditor projectPath={...} />                               │
...
**没有了**：自定义 protocol、独立窗口/WebContentsView overlay、Web Worker 扩展宿主、editor preload bridge、`editor-bundle` 静态产物、webpack4 构建。Monaco 由 vite 和其余 renderer 代码一起打包。
...
`src/main/ipc/project-fs.ts`（取代 OpenSumi 的 `editor:fs:*` bridge）：
...
- **sender policy**：用标准 `ctx.senderPolicy`（主窗口可信）。Monaco 在主 renderer，无需 OpenSumi 那套独立 view 的 allow-list。同步通道经 `IpcRegistry.handleSync` 同样受 policy 把关，且**任何**路径(policy 拒绝 / policy 自身抛 / handler 抛)都必落一个 `{ ok:false }` 哨兵到 `event.returnValue`——否则被阻塞的 renderer 会永久挂起。
...
- **WXML**：注册 `wxml` 语言 + `setMonarchTokensProvider(wxmlMonarchLanguage)` + 语言配置。
  ...
  - **wxml LSP**（`language/wxml-lsp.ts`，从旧 OpenSumi 集成**直接移植**——本就是纯 `monaco.languages.register*` API，只把 import 源从 `@opensumi/monaco-editor-core` 换成 `monaco-editor`）：`registerCompletionItemProvider`（标签名/属性名/属性枚举值/`wx:`指令/事件）+ `registerHoverProvider`（组件与属性文档）。元数据在 `language/wxml-data.ts`。
...
Monaco 由 vite 随 renderer 一起打包（`pnpm build:renderer`）；语言 worker 经 `?worker` 产出独立 chunk。**无**单独的 editor 构建步骤、**无** `editor-bundle` 产物、**无** editor preload（`build:preload` 已移除 `editor.cjs`）。`_spike/` 与 `editor-bundle/` 已删除。
...
| 4 | wxml LSP 无专测 | 旧 OpenSumi 的 `wxml-lsp.spec.ts` 已删；`wxml-lsp.ts` 是纯 `monaco.languages.register*` API，可补一个能在 vitest 跑的 provider 单测做回归网 |
...
| `.../monaco-editor/language/wxml-lsp.ts` | wxml completion/hover（移植自旧集成）|
...
| `src/renderer/.../project-runtime/layout/frame-tree.tsx` | editor leaf 渲染 cellNodes.editor（Monaco）|
| `src/renderer/.../project-runtime.tsx` | `cellNodes.editor = <MonacoEditor/>`，去掉 editor bounds binding |

> editor 在三宫格里的布局/显隐/拖拽不在本文 —— 见 [`project-window-layout.md`](./project-window-layout.md)。
