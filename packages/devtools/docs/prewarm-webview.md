...
> 配套：[`native-bridge-protocol.md`](./native-bridge-protocol.md)、[`workbench-model.md`](./workbench-model.md)、[`miniapp-snapshot.md`](./miniapp-snapshot.md)。
...
`PAGE_OPEN`（`handlePageOpen`）不创建新 `BrowserWindow`，它只在 router state 里 push 一个 `PageSession`，等 simulator 端 render-host `<webview>` 子 frame 自己上线（`ensureRenderBound` 在 `RENDER_INVOKE` 第一次到达时 bind sender）。所以**多页打开本身不开新进程**，预热对二次开页**收益接近零**。这是本设计区别于"Web 浏览器 tab 预热"的关键事实。

### 1.3 当前已经是热路径的部分

| 子系统 | 来源 | 备注 |
|---|---|---|
| miniapp session 配置 | `registerMiniappSessionConfigurator` / `configureMiniappSession`（`miniapp-partition.ts`）、`bridge-router.ts` 协议注册 | Referer/CORS + 协议 handler 经 partition configurator 在每个 partition 首用时应用一次（per-project `persist:miniapp-<key>` + 共享 fallback）；preload 改由各宿主 `webPreferences.preload` / `<webview preload>` 挂；pool 不会改这里 |
...
`ServiceHostSpec`— 一个 spec 完整决定一个 warm 窗口能否服务某调用方；复用 key 是 `preloadPath`：
...
进程模型三 flag 默认对齐 service-host（`createServiceHostWindow`，create.ts 的 `false / false / false`）：service-host preload 在 page realm 里 `require('electron')` 并写 globals，必须 `contextIsolation:false` 等。pool 不写死这三项，留给需要不同 runtime 的调用方覆盖。

`ServiceHostPoolStats`：`{ total, ready, inUse, warming, resetting, spec }` 占用快照。

`ServiceHostPoolInitOptions`：`{ defaultPoolSize, defaultSpec, maxPoolSize? }`，`maxPoolSize` clamp 到 ≤ 4（`HARD_MAX_POOL_SIZE`），默认 3。

### 3.3 公开方法

| 方法 | 行为 |
|---|---|
| `init(opts)` | 用 `defaultSpec` 预热到 `defaultPoolSize` 个 ready entry。`warmUpToTarget` 带迭代守护，持续 warm 崩溃下 best-effort（可能 under-filled）而非死循环 — init 后 `getStats().ready` 是建议值不是硬不变量 |
| `acquire(spec)` | 命中匹配 spec 的 ready entry → 置 in-use 返回 `{ win, entryId }`；spec 变更（preloadPath 不同）→ tear down 所有 pooled entry 并 re-target；池空 → 同步 fallback create，返回 `entryId === null` |
| `release(entryId, win)` | `entryId === null`（fallback 窗）或 spec 不再匹配 → 销毁；否则 reset（先导航 about:blank 后按 spec 清 storage）后重置 `ready`，超 `maxPoolSize` 则丢弃 |
| `releaseDestroyed(entryId)` | 调用方报告 acquire 来的窗口已**外部死亡**（优雅关闭 / 崩溃），回收其 in-use slot。对未知 id no-op；幂等；不触碰已消失的窗口。bridge-router 在 `serviceAlreadyClosed` 路径调用（该路径故意跳过 `release`） |
| `resize(target)` | re-target 到 `[0, maxPoolSize]`；超出 target 的 pooled entry 按**最旧优先**销毁；in-use 不动；不主动 warm 新 entry |
| `getStats()` | 返回 `ServiceHostPoolStats` 占用快照 |
| `dispose()` | tear down 所有 entry 并停止接活 |

### 3.4 reset 契约（release → ready）

reset（`reset`）顺序固定（先导航后清存储）：先 navigate 到 `about:blank` 让旧 document 停止运行、in-flight 请求 abort，**然后**按 spec 决定是否清 storage — 这样还在跑的旧页面不会在清空后又写回 storage。

下表是 **`clearStorageOnReset === true` 时**的 storage 清理契约（`RESET_STORAGES`, pool.ts + `clearCache()`）：
...
> **池窗在共享 session 下的实际行为**：注意这里说的是**池窗**——`serviceHostSpec()` 不带 appId 时落在共享 `persist:simulator` partition（= `SERVICE_HOST_PARTITION`），非池的 default 路径已 per-project 隔离（见 §1.2.1）。`serviceHostSpec()` 把 `clearStorageOnReset` 设为 **`false`**，所以池窗 `release` 时 `reset()` 在导航回 `about:blank` 之后**直接 early-return**，**不**调用 `clearStorageData` / `clearCache` — 否则若池窗碰巧落在共享 session 上会炸掉别的活项目 storage。跨 spawn 的隔离由"导航销毁 JS realm" + storage 在 persist 分区上持久化共同保证。
...
miniapp session 的 Referer/CORS + 协议 handler 改由 partition configurator（`registerMiniappSessionConfigurator` / `configureMiniappSession`，`miniapp-partition.ts`）在每个 per-project `persist:miniapp-<key>` partition（及共享 fallback）首用时应用一次；simulator WCV 的 preload 改由 `attachNativeSimulator` 经 `webPreferences.preload` 挂（不再 session 级 register）。这块不在本 pool 的优化范围。

#### 3.5.2 service 侧（`createServiceHostWindow`）

这是 pool 的主要价值实现处。`handleSpawn`的两条分支：

```
bridge-router.handleSpawn
  state.pool 非空（pool ON）：
    → pool.acquire(serviceHostSpec())   // bridge-router.ts
        → 池中有 ready entry → 立即返回 { win, entryId }
        → 池空 → fallback new BrowserWindow，返回 { win, entryId: null }
    → navigateServiceHost(win, buildServiceHostSpawnUrl({...}))  // bridge-router.ts
...
did-finish-load → boot 的接入要点：

- **pool 路径**：注册一个 **URL 过滤**的 `did-finish-load` 监听（`bootOnServiceLoad`），只在真正的 `service.html` 导航上 `bootServiceHost` — 过滤暖机 / fallback 的 `about:blank` did-finish-load。监听 ref 存到 `AppSession.onServiceBoot`，dispose 时先摘除（否则被回收的窗口会带着 stale 监听把已 dispose 的 session boot 进下一个 spawn）。
- **fresh 路径**：唯一导航就是 service.html（在 `createServiceHostWindow` 内发起），所以用自摘除的 `once('did-finish-load')`。
- **liveness guard**：`bootServiceHost`开头校验 `state.appSessions.get(ap.appSessionId) !== ap → return`，杜绝早 dispose 后暖窗复用时把上一个 app 的 `logic.js` 注入下一个窗口。

dispose 接入（`disposeAppSession`）：

- 池窗、非外部关闭：先摘 `onServiceClosed`+ `onServiceBoot`两个 per-spawn 监听，再 `pool.release(entryId, win)`。
- 池窗、`serviceAlreadyClosed`（窗口已外部 closed/crash，由 `onServiceClosed`, bridge-router.ts 进入此路径）：跳过 `release`，改 `pool.releaseDestroyed(entryId)`回收 in-use slot — 否则该 entry 永久泄漏在 pool 里、永久缩小容量。
- 非池窗：走原 `serviceWindow.close()`。
...
所有窗口死亡（render 崩溃 / 优雅关闭 / 调用方报告的外部销毁）都汇入 `reclaim`：

| 场景 | 处理 |
|---|---|
| `render-process-gone`（pooled 窗 ready 时崩溃） | `onGone`→ `reclaim({ destroyIfPooled: true, refill: true })` → 销毁残骸 + 补 1 个新 entry |
| `'closed'`（pooled 窗优雅关闭） | `reclaim({ refill: false })`（warmOne 注册的 once）→ 仅清 entry 表，**不 refill** |
| `release` 后 ready 窗发现 `isDestroyed()` | acquire 扫描时 `reclaim({ refill: false })` 丢弃，绝不外发死窗 |
| 调用方报告 in-use 窗已外部死亡 | `releaseDestroyed`→ `reclaim`，不触碰已消失窗口、不 refill |
| `loadURL` 失败（warm / fallback / reset） | `loadBlank`把 reject 和同步 throw 都吞成 settled，单个坏 warm 不 wedge 整个 pool |
| 并发 acquire 抢空 ready 池 | 第二个 acquire 扫不到 ready → 走 fallback create |

> **关键不变量**：refill 只在**崩溃恢复**（`render-process-gone → onGone`）发生，**不在优雅关闭**。pooled 窗的 `'closed'` 只在 app/项目 teardown 时出现（窗口隐藏，没别的东西会关它）— 此时若 refill 会在 Electron 退出途中 `new BrowserWindow`，造窗比关窗还快，挂死 shutdown。所以 `refill` 是 `reclaim` 的逐调用方 opt-in（pool.ts 注释）。
...
**JS 全局符号残留**：`DiminaServiceBridge`、`DiminaRenderBridge`、`__diminaSpawnContext` 是 preload 写入的全局符号。`reset()`**不** `executeJavaScript` 删 globals — 它单靠 navigate 回 `about:blank` 的跨 document 导航销毁整个 JS realm。（iOS 的 `prepareForReuse` 额外显式 `delete` 全局桥，dimina-kit 不照搬，因为跨 document 导航本身就重建了 realm。）

**共享 session storage（仅池窗）**：池窗的 spec `serviceHostSpec()`（无 appId）落在共享 `persist:simulator` session 上，且 `clearStorageOnReset === false`，所以池窗 reset **不**清这个共享 session 的 storage。注意这只对**池窗**为真——非池的 default 路径 `createServiceHostWindow` 已用 `miniappPartition(opts.appId)` per-project 隔离（见 §1.2.1）。带独立 session 的调用方（`clearStorageOnReset` 默认 true）才会触发 §3.4 的 storage 清理。
...
pool **默认 OFF**，仅当 `DIMINA_PREWARM_POOL_SIZE` 为正整数且 `DIMINA_PREWARM_DISABLE !== '1'` 时开启（`resolvePrewarmPoolSize` + `installBridgeRouter` 的 opt-in 检查）。这是唯一的配置契约（没有 settings.json 字段）。

| 变量 | 默认 | 作用 |
|---|---|---|
| `DIMINA_PREWARM_DISABLE` | unset | 设为 `1` 强制关闭 pool（即便设了 POOL_SIZE）；`handleSpawn` 全走 `createServiceHostWindow`，与未开池时一致 |
| `DIMINA_PREWARM_POOL_SIZE` | 未设置（=0=OFF） | 正整数开启 pool 并设池大小，clamp 到 ≤ 4（`PREWARM_MAX_POOL_SIZE`）；暖机延迟 500ms（避开冷启动） |
...
- [`native-bridge-protocol.md`](./native-bridge-protocol.md) — native-host Bridge 协议；service-host BrowserWindow 拓扑（pool 复用的目标窗口）。
