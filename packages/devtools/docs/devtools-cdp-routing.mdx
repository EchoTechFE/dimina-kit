...
> 这两条路径是 `installElementsForward` 与 `createNetworkForwarder` 两个独立服务，由 view-manager 各自接线；出站命令统一经过 elements-forward 安装的 `routeOutboundCommand` 闸口。
...
- **经共享 `CdpSessionBroker` 拿 render guest 会话**（`services/cdp-session/`）：safe-area、elements-forward、render-inspect、network-forward、console-forward 的 CDP injection 和 simulator-storage 六个消费者都从同一个 broker `acquire(wc)` 拿 lease。broker 是"谁 attach、谁能 detach"的唯一权威：一个 consumer 释放 lease 从不强制物理 detach，只有 broker 自己的顶层 `dispose()`（应用生命周期）才会 detach 它自己 attach 过的会话；外部 detach（真实 Chrome DevTools 抢占）或 wc 销毁会经 `lease.onDetach` 通知，consumer 借此自愈重新 acquire。`service-console` 监听的是 service-host，使用独立的直接 attach 路径。
...
<table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'system-ui, sans-serif', fontSize: '13px' }}>
  <thead><tr style={{ background: 'hsl(0 0% 14%)' }}>
    <th style={{ padding: '7px', textAlign: 'left', border: '1px solid hsl(0 0% 28%)' }}>当前支持</th>
    <th style={{ padding: '7px', textAlign: 'left', border: '1px solid hsl(0 0% 28%)' }}>当前结构不支持</th>
  </tr></thead>
  <tbody>
    <tr>
      ...
      <td style={{ padding: '8px', border: '1px solid hsl(0 0% 24%)', verticalAlign: 'top' }}>Console <code>$0</code> 求值（Runtime 落 service，拿不到 render 节点）<br/>Elements → Sources 跳转<br/>Network initiator 栈跳源<br/>样式编辑回写</td>
    </tr>
  </tbody>
</table>

> 支持右栏能力需要 `Target.attachedToTarget`、每后端 `sessionId` 和一套完整的 child-target 虚拟后端；这些都不在当前实现中。
...
> **Elements 路由两个要点**：
> 1. **复用、不抢占**：safe-area、elements-forward、render-inspect、network-forward、console-forward CDP injection 和 simulator-storage 经同一个 `CdpSessionBroker` 拿 debugger session lease；broker 内部决定复用已 attach 的还是自己 attach。consumer 不直接 `attach`/`detach`，不会释放其他 consumer 正在使用的会话。
