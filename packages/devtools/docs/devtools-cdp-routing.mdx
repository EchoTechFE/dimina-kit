...
| Network 注入 | `services/network-forward/` | `attachSimulator` + `attachRenderGuest` 各自抓事件、`DevToolsAPI.dispatchMessage` 注入前端；loadingFinished 预取 body/postData 入有界缓存（`bodies`，forwarder 级并发上限 + 大小预筛），供出站闸口回答 `dimina:sim:` id 的 `getResponseBody`/`getRequestPostData` |
