...
| 点源码 → Monaco | `view-manager` `injectOpenResourceHandler` → `shared/open-in-editor.ts` | 捕获阶段拦截 `.devtools-link` 点击 → `InspectorFrontendHost.openInNewTab(<sentinel>)`（旧 `setOpenResourceHandler` 在当前 Chromium 已移除，仅作降级 fallback） |
