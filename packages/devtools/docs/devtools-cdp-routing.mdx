...
| Console 汇聚（guest console sink） | `services/console-forward/` | 拥有 `consoleLog` sink：把 render 层（`source:'render'`）的日志重新打进 service-host 窗口的 `console`，使前端在 service-host 一处即可看到两层日志 |
