...
| 缩略图（含远程 host 路径）| `captureThumbnail` / `getThumbnail`（`workspace-service.ts`）。native-host 下截活跃 render-host guest（手机框内的 mini-program 内容），guest 不可用时降级截外层 simulator WCV |
