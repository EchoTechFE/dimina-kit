...
| 当前 session 打开 / 关闭 | `openProject` / `closeProject`（`workspace-service.ts`）|
| 编译配置读写 | `getCompileConfig` / `saveCompileConfig`（`workspace-service.ts`）|
| 缩略图（含远程 host 路径）| `captureThumbnail` / `getThumbnail`（`workspace-service.ts`）|

provider 注入：远程 host（如 qdmp 的云 workspace）通过 `ProjectsProvider` 接管 fs，默认走 `LocalProjectsProvider`，把 `<userData>/dimina-projects.json` 当后端（`shared/types.ts`、`services/projects/project-repository.ts`）。
...
detachSimulator()                              ← 关 project 时统一销毁
```
...
`detachSimulator`（`view-manager.ts`）销毁 simulator view，同时顺手销毁 native simulator view、hide popover、销毁 settings view、摘除 custom-apis 桥派发器。
