...
devtools 容器是一个 BrowserWindow（workbench 主舞台）+ 若干 WebContentsView 覆盖层（simulator DevTools / settings / popover）。mini-program 跑在主 renderer 内的 `<webview partition="persist:simulator">` 里，页面用 `<iframe>`（默认架构）或子 `<webview>`（native-host 架构）承载，两套运行时由 `DIMINA_NATIVE_HOST` 开关并存。所有 mini-program runtime 消息经 main 进程的 BridgeRouter（`src/main/ipc/bridge-router.ts`）这条总线编排 service ↔ simulator ↔ render 的两级 session（AppSession / PageSession）。
