...
| --- | --- | --- |
| `@dimina-kit/web-compiler/pool` | **推荐**：编排好的编译池——常驻 3-stage worker、stage 级并行、realm 复用，一个 `compile()` 拿产物 | **本包** |
| `@dimina-kit/web-compiler` / `./browser` | 底层 core 接缝（`compileMiniApp` + `setupCompile`/`compileStage`/`collectOutputs`/`resetCompilerState`），自定义编排时用 | 下游 |
...
```
   ...
   ┌──────────────────────────────────────────────┐
   │ @dimina-kit/web-compiler/pool  createCompilerPool
...
```js
import { createCompilerPool } from '@dimina-kit/web-compiler/pool'

const pool = createCompilerPool({
  // 起一个跑本包 stage worker 的 module worker(如何解析 URL 取决于你的 bundler/托管)
...
createCompilerPool(options: {
  createWorker: () => Worker      // 必填:起一个跑 dist/stage-worker.browser.js 的 module worker
...
import { Volume, createFsFromVolume } from 'memfs'
import { compileMiniApp } from '@dimina-kit/web-compiler/browser'
...
```bash
pnpm --filter @dimina-kit/web-compiler build          # node + 三个 browser bundle
pnpm --filter @dimina-kit/web-compiler build:browser  # 仅 browser(core + stage-worker + pool)
pnpm --filter @dimina-kit/web-compiler build:node     # 仅 node
```
...
```bash
pnpm --filter @dimina-kit/web-compiler test:node        # 编译 base 示例、校验产物
