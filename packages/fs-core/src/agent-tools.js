...
export function createAgentTools(fs) {
  ...
  const T = (name, description, inputSchema, execute, extra) => ({ name, description, inputSchema, execute, ...extra })
  ...
  const tools = [
    ...
    T('fs_restore', '回滚到指定 checkpoint（本身是一条可再撤销的写）；若 checkpoint 之后有人类编辑，默认拒绝并返回 restore-conflict{humanPaths}，需 force:true 确认覆盖', { cpId: 'string', force: 'boolean?' },
      ({ cpId, force }) => fs.restore(cpId, { ...agentOpts(), force }),
      { dangerous: true }),
    T('fs_diff', '列出某轮 turn 的全部改动（默认当前 turn）', { turnId: 'string?' },
      ({ turnId } = {}) => fs.diff(turnId || activeTurn)),
  ]
