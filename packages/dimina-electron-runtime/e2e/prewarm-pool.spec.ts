 ...
 * spawn hub — always on now), and warming starts ~500ms after app.ready
 * (installBridgeRouter), BEFORE a project is opened. So this launches with
...
test.describe('ServiceHostPool pre-warm (pool-ON) e2e', () => {
  ...
  test.beforeAll(async () => {
    ...
    electronApp = await _electron.launch({
      ...
      // launch (own dedicated Electron process) so it never leaks into other specs.
      env: {
        ...process.env,
        NODE_ENV: 'test',
  ...
  test('the app spawns + renders through the pooled service-host window', async () => {
    ...
    // — DeviceShell paints its per-page render webview.
