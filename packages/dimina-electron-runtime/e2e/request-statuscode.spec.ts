...
test.describe('service-layer wx.request HTTP status routing', () => {
  test('a 401 response reaches success with statusCode 401 (never fail)', async ({ electronApp }) => {
  ...
  test('a response slower than the legacy 5s router watchdog still arrives via success', async ({ electronApp }) => {
