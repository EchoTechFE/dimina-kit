 ...
 * dmcc rebuilds, and the render-host guest re-renders the page. We
 * deliberately do NOT assert an exact rebuild count: the rebuild-scheduler
 * coalesces writes that land within its debounce window into one trailing
 * build (see devkit's rebuild-scheduler.ts), so the only sound assertion is
 * "every round's marker eventually renders and the simulator stays alive
 * throughout" — not "exactly 30 builds ran".
 *
 * MANUAL-ONLY / opt-in: this test mutates the SHARED `fixtures/tabbar-app`
 * directory that several other specs in this package (native-host-render,
 * prewarm-pool, native-host-switchtab-rerender, …) read concurrently under
 * the default `pnpm test:e2e` full-suite run. Repeatedly rewriting a shared
 * fixture while those specs are reading/compiling it would race and produce
 * spurious failures unrelated to this test. It is gated behind
 * `HOT_RELOAD_STRESS=1` so it never runs as part of the default suite and
 * must be launched deliberately:
 *
 *   HOT_RELOAD_STRESS=1 pnpm --filter @dimina-kit/electron-runtime test:e2e hot-reload-stress
 *
 ...
 *
 * Split from devtools' original: that version also re-asserted devtools'
 * own panel/toolbar chrome (`assertPanelBasics` — "普通编译" button, the
 * "面板可见性" toggle group, WXML/AppData/Storage tab buttons) mid-run. This
 * harness has no such UI at all (a bare host window, no panels), so that
 * check is dropped — the leak/crash/render assertions below are the
 * genuinely runtime-owned behavior and don't need it.
 */
 ...
 * real rebuild+respawn+paint is expected to land in a couple of seconds, so
 * hitting this cap on every round would itself signal a real regression, not
 * just flakiness. */
const ROUND_TIMEOUT_MS = 15_000
/** Sub-budget between rewrite retries inside a round's timeout window — a
 * dropped chokidar inotify event gets a fresh write instead of just waiting
 ...
 * head/tail ratio are). Calibrated from a real 30-round run on devtools: the
 * browser process peaked around 211 MB and trended DOWN over the run, so ~2x
 * peak leaves generous headroom for slower/heavier machines while still
...
/**
 * Native readiness probe: `.device-shell-root` mounts only once
 * SimulatorMiniApp.spawn() resolves, and `.device-shell__webview` is
 * exclusive to the native render path.
 */
 ...
 * predicate keeps matching the same `marker`) — a dropped chokidar inotify
 * event gets a fresh write instead of a longer wait on an event that's
 * already gone.
 */
...
test.describe('Hot-reload stress (30 rapid recompiles)', () => {
  ...
  test.beforeAll(async () => {
    ...
    // while running headfully alongside other work.
