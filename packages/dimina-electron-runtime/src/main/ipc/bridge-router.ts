...
function rewriteSourceMappingUrl(source: string, scriptUrl: string): string {
  ...
  while ((m = re.exec(source)) !== null) {
    lastIndex = m.index + (m[1]?.length ?? 0)
    lastValue = (m[2] || '').trim()
  }
 ...
 */
function startLaunchTimer(state: RouterState, ctx: RuntimeContext, ap: AppSession): void {
 ...
 */
function markSessionRunning(ctx: RuntimeContext, ap: AppSession, page: PageSession): void {
...
function handleContainerMsg(
  ...
  switch (msg.type) {
    ...
    case 'storageChanged':
      ...
      ctx.events.emit('storage-changed', {
        appId: ap.appId,
        change: msg.body as SyncStorageChange,
      })
      break
    case 'wxmlChanged':
 ...
 */
function installAppLifecycleDriver(ctx: RuntimeContext, state: RouterState): void {
...
function resolveRootPagePath(
  ...
  const resolvedPagePath = manifest.pages.includes(manifest.entryPagePath)
    ...
    : (manifest.pages[0] ?? manifest.entryPagePath)
